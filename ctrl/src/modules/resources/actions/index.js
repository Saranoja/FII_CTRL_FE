...
export const postFileArticles = createPrefixedAction(actionTypes.POST_FILE_ARTICLES);
...
export const getResourcesForFile = (file, subjectId) => (dispatch) => {
    ...
    dispatch(
        postFile(
            POST(
                `${userManager.config.resources}/${subjectId}/pdf`,
                file,
                header,
            )
        )
...
export const getArticlesForFile = (file) => (dispatch) => {
    ...
    dispatch(
        postFileArticles(
            POST(
                `${userManager.config.articles}/pdf`,
                file,
                header,
            )
        )
