            ...
            <StyledDashboard>
                <Layout>
                    <div className="dashboard-content-wrapper">
                        <Grid container justify="center" spacing={2} className="dashboard-cards-stack">
                            {/* <Card.Group stackable itemsPerRow={3} doubling centered className="dashboard-cards-stack" > */}
                            {
                                R.map((column) => {
                                    return (<Grid item className="column" key={column.id}>
                                        <ActionCard image={column.first.image} title={column.first.title} description={column.first.description} route={column.first.route} />
                                        {column.second ? <ActionCard image={column.second.image} title={column.second.title} route={column.second.route} description={column.second.description} /> : null}
                                    </Grid>)
